...
class Character {
  constructor(x, y) {
    ...
    this.up = false;
    this.down = false;
    this.imageChangeTime = 0;  // Variable to store the time when the image change occurred
    this.imageChangeDuration = 300;  // Duration of the image change in milliseconds
    this.img = img_player
  }
  ...
  update() {
    ...
    if (this.left && this.x > x_point_gap_half + 1 && timer_left <= 1) {
      ...
      this.img = img_player_left
      this.imageChangeTime = millis();
    } else {
      timer_left = 0;
      x_plus_left = 0;
      p_x_plus_left = 0;
    ...
    if (if_hit_by_wind_right && abs(last_time_hit_by_wind - millis()) > 150) {
      key_input_cur = "L";
      if_hit_by_wind_right = false;
      last_time_hit_by_wind = millis();
      print("if_hit_by_wind");
    }
if (if_hit_by_wind_left && abs(last_time_hit_by_wind - millis()) > 150) {
  key_input_cur = "R";
  if_hit_by_wind_left = false;
  last_time_hit_by_wind = millis();
  print("if_hit_by_wind");
}
    if (millis() - this.imageChangeTime > 400) {
      this.img = img_player;
    }
  }
}
