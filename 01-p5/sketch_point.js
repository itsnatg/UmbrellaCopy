...
function point_image_preload() {
  ...
  img_player = loadImage("file/New-Player.gif");
  ...
  img_score05x = loadImage("file/pw_items/pw_items_score_half.png");
...
function point_setup() {
  ...
  let y_num = 0;
  for (
    let pos_Y = height / 18 + height / 9 + 40;
    pos_Y < height - (height / 9) * 2 - 1;
    pos_Y += y_point_gap
  ) {
    y_num++;
    x_num = 0;
...
class Point {
  constructor(x, y, serial, x_num, y_num) {
    ...
    this.test = "yes";
    this.size = item_size || random(width / 16 - 25, width / 16);
    ...
    if (this.if_visable) {
      ...
      switch (this.spot_kind) {
        ...
        case "snail":
          ...
          break;
        case "water":
          if (millis() - this.generate_time < 15000) {
            image(img_water_splash, 0, 0, this.size, this.size);
          } else if (millis() - this.generate_time < 22000) {
            push();
            //scale(sca)
            image(img_water_splash, 0, 0, this.size, this.size);
            pop();
